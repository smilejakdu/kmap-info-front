...
const ExcelInfoList = ({ excel_file_name, handleChangeExcelData }) => {
    ...
    return (
        <div>
                ...
                <FileListBody>
                    {excel_file_name.map((item) => (
                        <FileItemBox key={item.id}>
                            <ExcelName onClick={ExcelNameClick(item.name)}>{item.name}</ExcelName>
            ...
            <RightBody>
                {excelName ? <RightFileName>{excelName}</RightFileName> : <RightFileName>File Name.xlsx</RightFileName>}
                {sheetData.length > 0 ? (
                    <RightSheetClick>
                        {sheetData.map((sheet, i) => (
                            <div>
                                {sheetName === sheet ? (
                                    <div key={i} className="active" onClick={getSheets(excelName, sheet)}>
                                        {sheet}
                                    </div>
                                ) : (
                ...
                <SheetTableData>{dataLoaded && <SheetTable cols={cols} rows={rows} />}</SheetTableData>
            </RightBody>
        </div>
    );
};
